...
class MainUtils:
    ...
    def get_model_score(test_y: DataFrame, preds: DataFrame) -> float:
        ...
        try:
            ...
            logging.info("Exited to get the model score method of MainUtils class")
            return model_score
    ...
    @staticmethod
    def get_model_score(model_name, str) -> object:
        ...
        try:
            ...
            logging.info("Exited to get_base_model score method of MainUtils class")
            return model
    ...
    def get_model_params(            
        ...
        try:
            ...
            model_name = model.__class__.__name
            model_config = self.read_yaml_file(filename=MODEL_CONFIG_FILE)
            model_params_grid = model_config["train_model"][model_name]
            model_grid = GridSearchCV(
                model, model_params_grid, verbose=VERBOSE, cv=CV, n_jobs =N_JOBS
            )
            model_grid.fit(x_train, y_train)
            logging.info("Exited the get_model_params method of the GridSearchCV")
    ...
    def save_object(file_path: str, obj: object) -> None:
        logging.info("Entered save_object method of MainUtils class")
        try:
            with open(file_path, 'wb') as file_obj:
                dill.dump(obj, file_obj)
            logging.info("Exited save_object method of MainUtils class")
    ...
    def load_model(file_path: str) -> object:
        logging.info("Entered the load_model method of MainUtils class")
        try:
            with open(file_path, "rb") as file_obj:
                obj = dill.load(file_obj)
            logging.info("Exited the load_model method of MainUtils class")
            return obj
    ...
    @staticmethod
    def udate_model_score(self, best_model_score: float) -> None:

        logging.info("Entered the udate_model_score method of MainUtils class")
        try:
            model_config = self.read_yaml_file(filename = MODEL_CONFIG_FILE)
            model_config["base_model_score"] = str("base_model_score")
            with open(MODEL_CONFIG_FILE, 'w+') as fb:
                safe_dump(model_config, fb, sort_keys=False)
                
            logging.info("Exited the udate_model_score of MainUtils class")
